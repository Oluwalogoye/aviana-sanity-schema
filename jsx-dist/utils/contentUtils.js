...
export const fetchMenuItems = async menuItems => {
  const refinedMenuItems = await Promise.all(menuItems.map(menuItemRef => {
    return fetchMenu(menuItemRef._ref).catch(error => {
...
export const handleFetchedContent = async (typename, content) => {
  if (typename === 'newsComponent' && content.newsItems) {
    ...
    const newsItemsContent = await Promise.all(content.newsItems.map(newsItem => {
      return fetchFurtherContent(newsItem._ref, typename).catch(error => {
    ...
    return {
      furtherContent: newsComponentContent
    };
    ...
    const featuredCompaniesContent = await Promise.all(content.featuredCompanies.map(featuredCompany => {
      return fetchFurtherContent(featuredCompany._ref, typename).catch(error => {
    ...
    return {
      furtherContent: featuredComponentContent
    };
  } else if (typename === 'frame' && content.featuredItems) {
    const frameContent = {
      featuredItems: []
    };
    const featuredItemsContent = await Promise.all(content.featuredItems.map(featuredItem => {
      return fetchFurtherContent(featuredItem._ref, typename).catch(error => {
    ...
    const framedItemsContent = await Promise.all(content.framedItems.map(framedItem => {
      return fetchFurtherContent(framedItem._ref, typename).catch(error => {
    ...
    return {
      furtherContent: framedCallToActionContent
    };
    ...
    const enhancedItemsContent = await Promise.all(content.enhancedItems.map(enhancedItem => {
      return fetchFurtherContent(enhancedItem._ref, typename).catch(error => {
    ...
    return {
      furtherContent: enhancedCallToActionContent
    };
    ...
    const pressReleasesContent = await Promise.all(content.pressReleases.map(pressRelease => {
      return fetchFurtherContent(pressRelease._ref, typename).catch(error => {
    ...
    return {
      furtherContent: pressSectionContent
    };
  } else {
    return {
      content
